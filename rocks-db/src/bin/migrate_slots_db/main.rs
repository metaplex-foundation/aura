...
async fn main() {
    ...
    let mut iter = slot_storage.db.raw_iterator_cf(
        &slot_storage.db.cf_handle(RawBlock::NAME).expect("get raw blocks cf handle"),
    );
    ...
    while iter.valid() {
        ...
        let raw_block_deprecated = match RawBlockDeprecated::decode(v) {
            Ok(block) => block,
            Err(e) => {
                if RawBlock::decode(v).is_ok() {
                    continue;
                } else {
                    panic!(
                        "found bad value which could not be decoded neither into the deprecated nor to up to date item! slot: {}, data (base58): {}, error: {}",
                        slot,
                        solana_sdk::bs58::encode(v).into_string(),
                        e
                    );
                }
            },
        ...
        if batch.len() < WRITE_BATCH_SIZE {
            ...
            slot_storage.raw_blocks_cbor.put_batch(map).await.expect("raw blocks batch");
        }
        iter.next();
    }
