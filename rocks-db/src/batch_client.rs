use crate::asset::{self, AssetIndex, AssetsUpdateIdx};
...
impl AssetUpdateIndexStorage for Storage {
    ...
    fn fetch_asset_updated_keys(
        ...
        let mut iterator = match last_key {
            Some(key) => {
        ...
        while let Some(pair) = iterator.next() {
            ...
            unique_pubkeys.insert(decoded_key.2.clone());
...
impl AssetIndexReader for Storage {
    async fn get_asset_indexes(&self, keys: &[Pubkey]) -> Result<HashMap<Pubkey, AssetIndex>> {
        ...
        for authority_info in asset_authority_details.iter() {
            if let Some(data) = authority_info {
                if let Some(existed_index) = asset_indexes.get_mut(&data.pubkey) {
        ...
        for owner_info in asset_owner_details.iter() {
            if let Some(data) = owner_info {
                if let Some(existed_index) = asset_indexes.get_mut(&data.pubkey) {
        ...
        for collection_info in asset_collection_details.iter() {
            if let Some(data) = collection_info {
                if let Some(existed_index) = asset_indexes.get_mut(&data.pubkey) {
