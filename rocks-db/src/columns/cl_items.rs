...
use crate::{
    column::TypedColumn,
    key_encoders::{decode_pubkey_u64, decode_u64_pubkey, encode_pubkey_u64, encode_u64_pubkey},
            ...
            let new_val = match bincode::deserialize::<SourcedClItemDeprecated>(op) {
                ...
                Err(_e_sourced) => {
                    ...
                    match bincode::deserialize::<ClItemDeprecated>(op) {
                        Ok(ci) => {
                            SourcedClItemDeprecated { item: ci, is_from_finalized_source: false }
                        },
                        Err(e_clitem) => {
...
impl Storage {
    ...
    pub(crate) fn save_changelog_with_batch(
        ...
        for p in change_log_event.path.iter() {
            ...
            if let Err(e) =
                self.cl_items.merge_with_batch(batch, ClItemKey::new(node_idx, tree), &cl_item)
            {
