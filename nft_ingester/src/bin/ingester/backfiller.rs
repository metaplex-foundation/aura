    ...
    async fn process_slot(
        ...
        let block = match producer.get_block(slot).await {
            ...
            Err(err) => {
                ...
                metrics.inc_data_processed("slots_parsed_failed_get_total");
                return;
            }
        };
        ...
        if let Err(err) = consumer.consume_block(block).await {
            ...
            metrics.inc_data_processed("slots_parsed_failed_total");
            return;
        }
        metrics.inc_data_processed("slots_parsed_success_total");
