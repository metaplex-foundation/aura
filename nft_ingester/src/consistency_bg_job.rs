...
use crate::consistency_calculator::{get_calculating_acc_epoch, get_calculating_bbgm_epoch};
...
pub fn run_consistenct_bg_job(
    ...
    tokio::spawn(async move {
        let _ = run_peers_checking_loop(storage, peers_provider).await;
    });
}
...
async fn run_peers_checking_loop(
    ...
    loop {
        ...
        let bbgm_task = {
            ...
            tokio::spawn(async move {
                exchange_bbgms_with_peers(epoch_to_process, storage, peers_provider).await
            })
        };
        let acc_task = {
            let storage = storage.clone();
            let peers_provider = peers_provider.clone();
            tokio::spawn(async move { exchange_account_with_peers(storage, peers_provider).await })
        };
...
async fn exchange_bbgms_with_peers(
    ...
    for (peer_ind, trusted_peer) in trusted_peers.iter().enumerate() {
        ...
        let Ok(client) = BbgmConsistencyApiClientImpl::new(trusted_peer)
            .await
            .map(Arc::new)
        else {
        ...
        let changes_we_miss =
            compare_bbgm_with_peer(grand_epoch, storage.as_ref(), client.as_ref()).await;
    ...
    handle_missing_bbgm_changes(missing_bbgm_changes, trusted_peers, storage).await
}
    ...
    for (change, peers) in missing_accounts {
        ...
        while let Some(peer_ind) = it.next() {
            ...
            if let Ok(block) = client
                ...
                .await
            {
                let _ = storage.consume_block(change.slot, block).await;
            }
            break;
        }
    }
...
async fn exchange_account_with_peers(
    ...
    for (peer_ind, trusted_peer) in trusted_peers.iter().enumerate() {
        ...
        let Ok(client) = AccConsistencyApiClientImpl::new(trusted_peer)
            .await
            .map(Arc::new)
        else {
            tracing::warn!("Cannot connect to peer={trusted_peer}");
        ...
        let accs_we_miss: Vec<AccLastChange> =
            compare_acc_with_peer(storage.as_ref(), client.as_ref()).await;
