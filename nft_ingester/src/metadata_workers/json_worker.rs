                ...
                Err(json_err) => {
                    ...
                    match json_err {
                        ...
                        JsonDownloaderError::ErrorStatusCode(err) => {
                            ...
                            {
                                info!(%metadata_url,%err, "Rate limited or timeout");
                            }
...
fn parse_cache_control_response(cache_control: &str) -> CacheControlResponse {
    ...
    let cache_control = cache_control.strip_prefix("cache-control:").unwrap_or(cache_control);
    for directive in cache_control.split(',').map(|s| s.trim()) {
        ...
        if let Some(stripped) = directive.strip_prefix("max-age=") {
            ...
            mutability = Some(OffchainDataMutability::Immutable);
        } else if directive == "mutable" {
            mutability = Some(OffchainDataMutability::Mutable);
        }
    }

    CacheControlResponse { max_age, mutability }
}
...
mod tests {
    ...
    #[test]
    fn test_parse_cache_control_max_age_and_mutability() {
        let cache_control = "cache-control: public, max-age=29030400, immutable";
        let result = parse_cache_control_response(cache_control);

        assert_eq!(result.max_age, Some(29030400));
        assert_eq!(result.mutability, Some(OffchainDataMutability::Immutable));
    }

    #[test]
    fn test_parse_cache_control_response_with_max_age() {
        let cache_control = "cache-control: public, max-age=29030400";
        let result = parse_cache_control_response(cache_control);

        assert_eq!(result.max_age, Some(29030400));
        assert_eq!(result.mutability, None);
    }

    #[test]
    fn test_parse_cache_control_response_with_mutability() {
        let cache_control = "cache-control: mutable";
        let result = parse_cache_control_response(cache_control);

        assert_eq!(result.max_age, None);
        assert_eq!(result.mutability, Some(OffchainDataMutability::Mutable));
    }
}
