...
mod tests {
    ...
    async fn test_calc_acc_nft_checksums() {
        ...
        let expected_bucket0_checksum = {
            ...
            let acc_bytes = bincode::serialize(&latest_acc1_val).unwrap();
            hasher.hash(&AccountNft::clear_slot_bytes(&acc_bytes));
            hasher.result().to_bytes()
        };
        let expected_bucket1_checksum = {
            let mut hasher = Hasher::default();
            hasher.hash(&AccountNft::encode_key(latest_acc2_key.clone()));
            let acc_bytes = bincode::serialize(&latest_acc2_val).unwrap();
            hasher.hash(&AccountNft::clear_slot_bytes(&acc_bytes));
            hasher.result().to_bytes()
        };
        assert_eq!(
    ...
    async fn test_notification_on_epoch_change() {
        ...
        let sut = NftChangesTracker::new(Some(sender));
