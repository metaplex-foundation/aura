...
mod tests {
    ...
    use interface::{solana_rpc::MockTransactionsGetter, signature_persistence::{MockSignaturePersistence, MockTransactionIngester}};
    use metrics_utils::RpcBackfillerMetricsConfig;
    use mockall::predicate::{eq, self};
    ...
    async fn test_fetch_signatures_with_over_batch_limit_elements_should_complete_without_infinite_loop() {
        ...
        let slot = 1;
        let signature_with_slot = super::SignatureWithSlot {
            signature,
            slot,
        };
        ...
        data_layer.expect_first_persisted_signature_for().with(eq(program_id)).times(1).return_once(move|_|Ok(Some(sig_clone)));
        rpc.expect_get_signatures_by_address().with(eq(signature_with_slot), eq(program_id)).times(1).return_once(move|_, _|Ok(signatures));
        data_layer.expect_missing_signatures().with(eq(program_id), predicate::always()).times(2).returning(move|_, _|Ok(vec![]));
        data_layer.expect_drop_signatures_before().with(eq(program_id), predicate::always()).times(1).returning(move|_, _|Ok(()));
        let fetcher = super::SignatureFetcher::new(Arc::new(data_layer), Arc::new(rpc), Arc::new(ingester), metrics);
