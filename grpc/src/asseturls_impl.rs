...
use crate::asseturls::{AssetsToDownload, DownloadError, DownloadResultsRequest, DownloadSuccess, GetAssetUrlsRequest, UrlDownloadDetails};
use std::sync::Arc;
use interface::assert_urls::{UrlsToDownloadStore, DownloadOutcome, UrlDownloadNotification};
...
impl AssetUrlService for AssetUrlServiceImpl {
    ...
    async fn get_asset_urls_to_download(
        ...
        tonic::Response<AssetsToDownload>,
        tonic::Status,
    > {
    ...
    async fn submit_download_result(
        ...
        let notifications = request.into_inner().results.into_iter()
...
impl From<DlResult> for DownloadOutcome {
    fn from(value: DlResult) -> Self {
        match value {
            ...
            DlResult::Fail(fail) => {
                match DownloadError::try_from(fail) {
                    Ok(download_error) =>
