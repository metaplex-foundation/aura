...
impl PgClient {
    ...
    pub(crate) async fn recreate_asset_indexes(
        ...
        for (index, on_query_string) in [
                ...
                ("idx_assets_v3_owner_pubkey", "assets_v3 (ast_owner, ast_pubkey DESC)"),
    ...
    pub(crate) async fn recreate_asset_constraints(
        ...
        .await?;
        let mut query_builder: QueryBuilder<'_, Postgres> = QueryBuilder::new("ALTER TABLE assets_v3 ADD CONSTRAINT assets_v3_ast_metadata_url_id_fkey FOREIGN KEY (ast_metadata_url_id) REFERENCES tasks(metadata_hash) ON DELETE RESTRICT ON UPDATE CASCADE NOT VALID;");
        self.execute_query_with_metrics(
            transaction,
            &mut query_builder,
            ALTER_ACTION,
