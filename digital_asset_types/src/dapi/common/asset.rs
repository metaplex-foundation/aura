...
pub fn complement_asset_content_with_json_data(content: &mut Content, metadata: Value) {
    let mut selector_fn = jsonpath_lib::selector(&metadata);
    let selector = &mut selector_fn;

    let desc = safe_select(selector, "$.description");
    if let Some(desc) = desc {
        content.metadata.set_item("description", desc.clone());
    }

    let symbol = safe_select(selector, "$.attributes");
    if let Some(symbol) = symbol {
        content.metadata.set_item("attributes", symbol.clone());
    }
